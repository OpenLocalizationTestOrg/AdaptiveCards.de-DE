...
Alle `actions` innerhalb der Karte werden als WPF `Button`s angezeigt, aber es liegt an Ihrer APP, zu behandeln, was geschieht, wenn ein Benutzer Sie drückt. 

Das `RenderedAdaptiveCard` -Objekt stellt `OnAction` für diesen Zweck ein Ereignis bereit.
